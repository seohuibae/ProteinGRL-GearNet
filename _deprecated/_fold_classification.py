...
class ProtClassHomologyDataSet:
    ...
    def __init__(self, pDataset = "training", ROOT_DIR='../data-downstream/', 
        pRandSeed = None, pPermute = True, pAmino = False, pLoadText = False):
        """Constructor.
        """
        pPath="../data-downstream/HomologyTAPE"
        ...
        with open(pPath+"/"+pDataset+".txt", 'r') as mFile:
            for curLine in mFile:
                ...
                curClass = self.classes_[splitLine[-1]] # class label 
                self.fileList_.append(pPath+"/"+pDataset+"/"+splitLine[0]) # domain na,e 
